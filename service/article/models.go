...
// FindOne 查询
func FindOne(condition interface{}) (Article, error) {
	var model Article
...
func FindCondition(condition interface{}, page common.Page) (interface{}, error) {
	...
	var models []Article
	var count int
	err := helper.DB.Model(&Article{}).Where(condition).Count(&count).Error
	if err != nil {
		return result, err
	}
...
func PatchOne(data interface{}) error {
	var article Article
	err := helper.DB.Model(&article).Updates(data).Error
	return err
}
